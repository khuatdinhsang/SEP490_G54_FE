    ...
    return (
        <SafeAreaView style={styles.container} >
            <HeaderNavigatorComponent
                ...
                handleClickIconRight={viewResult}
            />
            <View style={[flexCenter, styles.content, { flexDirection: 'column' }]}>
                <Image
                    source={IMAGE.PLAN_MANAGEMENT.PEN_BOOK} />
                <Text style={{ fontWeight: '700', fontSize: 24, color: colors.black, marginBottom: 10 }}>{t("evaluate.thankyouSurvey")}</Text>
                <Text style={{ fontWeight: '400', fontSize: 18, color: colors.gray_G06 }}>{t("evaluate.viewChartResult")}</Text>
            </View>
            <Pressable onPress={viewResult} style={[styles.button, { backgroundColor: colors.primary }]} >
                <Text style={styles.textButton}>{t("evaluate.viewResult")}</Text>
            </Pressable>
        </SafeAreaView >
    );
};
