...
const SF_ACTIVITY = ({ route }: any) => {
    const navigation = useNavigation<NativeStackNavigationProp<ParamListBase>>();
    const { t } = useTranslation();
    const time = route?.params?.time
    const dataSF_MENTAL = route?.params?.data
    ...
    const nextPage = () => {
        if (reviewMode) {
            navigation.navigate(SCREENS_NAME.EVALUATE.SF_DIET, { time, reviewMode: true });
        } else {
