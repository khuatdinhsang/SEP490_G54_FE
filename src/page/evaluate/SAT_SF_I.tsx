...
const SAT_SF_I = ({ route }: any) => {
    ...
    useEffect(() => {
        const getListQuestion = async () => {
            ...
            try {
                ...
                const res = await monthlyQuestionService.getListQuestion(TypeQuestion.SAT_SF_I, lang)
                if (res.code === 200) {
                    setErrorMessage("");
                    setIsLoading(false)
        ...
        const getListQuestionResult = async () => {
            ...
            try {
                ...
                const langAys = await AsyncStorage.getItem("language")
                const lang = langAys === 'en' ? LANG.EN : LANG.KR
                const res = await monthlyQuestionService.getResultListQuestion(time, TypeQuestion.SAT_SF_I, lang);
                if (res.code === 200) {
                    setErrorMessage("");
    ...
    const nextPage = () => {
        if (reviewMode) {
            navigation.navigate(SCREENS_NAME.EVALUATE.SF_MENTAL, { time, reviewMode: true });
        } else {
